...
namespace BAL.BL
{
	public class AddProgramsBL
	{
		...
		public SqlDataReader GetGroups(int categoryId, string catScheduleId)
		{
			string query = @"select distinct b.Branch_Id,b.Branch_Name, b.Branch_T_Years, b.Branch_Srt_Date, CONCAT(b.Branch_Name,':Year',b.Branch_T_Years, ':' , b.Branch_Srt_Date,'-',b.Branch_End_Date)
							 as Group_Year_Sem_Schedule 
							 from tbl_Branch b
							 join tbl_Schedule s on b.Branch_Id = s.Branch_Id
							 where Branch_Srt_Date is NOT NULL AND b.Category_Id= " + categoryId + " AND s.Schedule_ID = '" + catScheduleId + "'";
			return ConnectionFactory.ExecuteCommand(query, CommandType.Text);
		}
		...
		public DataTable GetCategorySubjectDetails(int categoryId, string categoryScheduleId, int branchId)
		{
			string query = string.Format("Select * from SubjectsView where Category_Id = {0} AND ScheduleId = \'{1}\' AND Branch_Id = {2};", categoryId, categoryScheduleId, branchId);
        ...
        public DataTable GetMandatoryGroupSubjects(int categoryId, string categoryScheduleId, int branchId)
        {
            StringBuilder sbquery = new StringBuilder();
            sbquery.Append("SELECT distinct sc.Subject_Id , sb.Subject_Name from tbl_Schedule sc ");
            sbquery.Append("JOIN tbl_Subject sb on sc.Subject_Id = sb.Subject_Id AND sc.Branch_Id = sb.Branch_Id AND sc.Category_Id = sb.Category_Id");
            sbquery.AppendFormat(" WHERE sc.Category_Id = {0} AND sc.Schedule_ID = \'{1}\' AND sc.Branch_Id = {2};", categoryId, categoryScheduleId, branchId);
            SqlDataReader dr = ConnectionFactory.ExecuteCommand(sbquery.ToString(), CommandType.Text);
            DataTable dt = new DataTable();
            dt.Load(dr);
            return dt;
        }
