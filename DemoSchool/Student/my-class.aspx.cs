...
{
    public partial class my_class : System.Web.UI.Page
    {
        ...
        private void SubjectFullInfo()
        {
            ...
            if (dtSubject.Rows.Count > 0)
            {
                ...
                txtSCASCMP.Value = txtCompanyName.Value;
             //   txtSCCMPLTDUNITS.Value = dtSubject.Rows[0][""].ToString();
                txtSCDURATIION.Value = dtSubject.Rows[0]["Duration"].ToString();
                txtSCENDDATE.Value = dtSubject.Rows[0]["Schedule_end_Date"].ToString();
                 txtSCFCLTYNAME.Value = dtSubject.Rows[0]["Faculty"].ToString();
              //  txtSCFCLTYSTATUS.Value = dtSubject.Rows[0][""].ToString();
              //  txtSCFCLTYSTATUSDATE.Value = dtSubject.Rows[0][""].ToString();
                txtSCLOC.Value = txtCompanyLocation.Value;
                txtSCNAME.Value = dtSubject.Rows[0]["Subject_Id"].ToString() + ":" + dtSubject.Rows[0]["Subject_Name"].ToString();
                txtSCSBJUNITS.Value = dtSubject.Rows[0]["Subject_T_Units"].ToString();
                txtSCSTARTDATE.Value = dtSubject.Rows[0]["Schedule_srt_Date"].ToString();
                txtSCSTATUSDATE.Value = dtSubject.Rows[0]["Status_date"].ToString();
                txtSCTRCLSREMARKS.Value = dtSubject.Rows[0]["Remarks"].ToString();
                txtSCTRCLSTYPE.Value = dtSubject.Rows[0]["ClassType"].ToString();
                txtSCUNITS.Value = dtSubject.Rows[0]["Subject_T_Units"].ToString();



                BindStudentGVDetails(dtSubject.Rows[0]["Schedule_ID"].ToString());
            }
        ...
        {
            SqlDataReader dr = objStudentBL.GetStudentsUnitsBySchedule(ScheduleID, Session["UserID"].ToString(), Convert.ToInt32(ddlSubject.SelectedValue.Trim()));
            DataTable dt = new DataTable();
            dt.Load(dr);
            if (dt.Rows.Count > 0)
            {
