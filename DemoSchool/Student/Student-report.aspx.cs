    ...
    public partial class Student_report : System.Web.UI.Page
    {
        ...
        private void CategoryDetails()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                BindStudentGVDetails(dt.Rows[0]["Schedule_ID"].ToString());
        ...
        {
            SqlDataReader dr = objStudentBL.GetUnitsScheduleBySubjects(ScheduleID,"Ramesh",Convert.ToInt32(ddlSubjects.SelectedItem.Value));
            DataTable dt = new DataTable();
            dt.Load(dr);
            if (dt.Rows.Count > 0)
            {
